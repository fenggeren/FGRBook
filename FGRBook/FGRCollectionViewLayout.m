...
const static CGFloat maxScale = 1.05;
...
@implementation UICollectionViewLayoutAttributes(FGRExtension)
@dynamic shadowRadius;

- (CGFloat)shadowRadius
{
    NSNumber *shadow = objc_getAssociatedObject(self, "shadowRadius");
    if (!shadow) {
        objc_setAssociatedObject(self, "shadowRadius", @0, OBJC_ASSOCIATION_RETAIN);
    }
    return shadow.floatValue;
}

- (void)setShadowRadius:(CGFloat)shadowRadius
{
    objc_setAssociatedObject(self, "shadowRadius", @(shadowRadius), OBJC_ASSOCIATION_RETAIN);
}
